...
2. **Using Linkings.** Link the same voices that are connected, such as `United States students`, two `/s/` sounds become one `/s:/` sound but a bit longer;
3. **Dropping sounds.** To keep fluency in rapid speech, we always drop explosive sounds between two consonants, such as `an important role` becomes `an importan role`;
4. **Holding sounds.** Similarly, we always hold explosive sounds but do not release them when these sounds between a vowel and a consonant, such as `that person` becomes `tha-person`;
5. **Linking vowels.** When a word ends with explosive sounds like `/t/`, `/k/`, and `/p/` and the following word begins with a vowel sound, we always link it together, such as `that element` becomes `tha-telement`.
6. **Reducing /h/ sounds.** When a word ends with explosive sounds and the following word begins with `/h/` sound, we always reduce the `/h/` sound, such as "might have" becomes "migh-tave" and "an hour" becomes "a-nour."

There are lots of rules that we should learn and practice, some beginners might feel overwhelmed, however, this is a vital part if we want to speak like a native and sound natural. Keep learning from online resources and practice consistently until you feel comfortable.
...
In particular, we always break sentences down before the preposition and never separate between compound nouns. 

Furthermore, when practicing this strategy, I suggest that we can separate fragmentarily, like this: 
...
Now we know what is the term "Thought groups" and how to divide a sentence, the next step is how to present this concept, it is related to the term "intonation". Intonation means the words are sounded in a higher pitch or a lower pitch.
...
Despite numerous difficulties on the test day, such as we are disrupted by other testers or meeting a word that have never seen, I mainly suggest we should speak slowly and confidently.

This is a trade-off, when we face a word or phrase that is hard to express and make hesitations simultaneously, we may lose scores of Pronunciation and Fluency, while if we express them slowly and confidently that keep the natural flow, we only lose Pronunciation's score. That is why I strongly maintain that we should speak confidently even making mistakes.

## Summary

This article mainly records knowledge that I learned through the English learning journey, including methods of how to rectify our pronunciation and concepts of PTE's criteria of the speaking module. 

Additionally, I decided to write posts in English from now on, this post is my first try. Although it may consist of numerous grammatical errors and awkward sentences, I must take a step forward. "Practice makes perfect" is what I learned throughout this journey.