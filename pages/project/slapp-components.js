    ...
    <>
      ...
      <div className="layout project-v2 flex flex-col gap-6">
        ...
        <section className="content">
          ...
          <ProjectImage src="https://lrdim.oss-cn-shenzhen.aliyuncs.com/projectimg/slapp-components/w_1920/3-1.png" />
          <p>
          ...
          <p>
            App
            有一段时间需求量极多，在做业务需求的同时我逐渐发现有几种模式是特别常用的，比如从用
          ...
          <p>
            B
            端业务表单比较多，我当时在做到输入框组件时发现了对「校验时机」这个课题的知识储备还比较缺乏，无法提供到相对有说服力的方案，于是我便对「App
            表单校验时机」进行了相对理性的探索。
          </p>
          <ProjectImage src="https://lrdim.oss-cn-shenzhen.aliyuncs.com/projectimg/slapp-components/w_1920/4-2.png" />
          <p>
          ...
          <p>
            在设计侧完成，且与其他业务设计师同步过后，也需要跟研发侧同步。当时在做
            app 需求时，设计师与研发侧合作的现状是这样的：
          </p>
          <ol>
            <li>
              设计师对 app
              内已有的组件模糊不清。不清楚现在有什么样的组件。经常会问到「XX
              功能有做成组件吗？」、「在哪里能看到？」等问题；
            </li>
            <li>
              方案中复用某个组件时交流成本较大。设计师计划使用一个组件时，给到的描述时「复用
              XX 功能下 XX 页面的 XX
              组件」。同一个业务内复用时或许还能勉强沟通，没有参与到组件的设计和研发用这种方式沟通效率较低；
            </li>
            <li>
              验收中需要重复标注问题，开发也要重复修复样式问题。样式相同的控件却需要花额外的时间去验收和标注，设计师和研发双方都耗费了时间和精力。
            </li>
          </ol>
          <p>
            为了改善现状，我推动客户端的研发在测试包里做一个组件库的 Demo
            入口，里面会按照类型聚合所有组件。对研发来说工作量是将组件复制多一份。但由于很现实的原因，做这个事情需要工时和排期，所以只能在每个版本节奏中抽空实现，所以我也列了一份优先级清单，先做高频、重要的组件。
          </p>
